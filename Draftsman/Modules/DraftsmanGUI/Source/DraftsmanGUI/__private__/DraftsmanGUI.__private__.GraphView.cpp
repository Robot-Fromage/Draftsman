...
GraphView::GraphView( QWidget* iParent ) :
    tSuperClass( iParent ),
    ...
    mVignette_TopLeft(  NULL ),
    mVignette_Top(      NULL ),
    mVignette_TopRight( NULL ),
    mVignette_Right(    NULL ),
    mVignette_BotRight( NULL ),
    mVignette_Bot(      NULL ),
    mVignette_BotLeft(  NULL ),
    mVignette_Left(     NULL )
{
    Init();
    Build();
...
{
    QColor background_color = getBackgroundColor();
    QColor grid_color       = getGridColor();
    ...
    // Painting grid
    int gridSizei = getGridSize();
    if( gridSizei <= 0 ) gridSizei = DEFAULT_GRID_SIZE;

    int subgridCounti = getSubGridCount();
    if( subgridCounti <= 0 ) subgridCounti = 1;

    if( Scale() < 0.5f )
        gridSizei = gridSizei * 2;

    drawGrid( painter, rect, getGridColor(), getSubGridColor(), gridSizei, subgridCounti );
}


void
GraphView::drawForeground(QPainter *painter, const QRectF &rect)
{
...
void
GraphView::drawGrid( QPainter* painter, const QRectF &rect, const QColor& iGridColor, const QColor& iSubGridColor, int iGridSize, int iSubGridCount)
{
...
void
GraphView::DeltaScale( qreal iDelta )
{
...
void
GraphView::SetScale( qreal iValue )
{
    QTransform t = transform();
...
GraphView::Build()
{
    setObjectName( "DraftsmanGUIGraphView" );
